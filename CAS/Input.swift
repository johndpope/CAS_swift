...
func convert(arg: [AnyObject]) -> Basic {
    ...
    var tempout = arg
    let operations:[Character] =  Array(")f^-*/+")
    for i in operations {
        if i == "f" {
            tempout = strFuncs(tempout)
        } else {
...
func strExtract(arg: [AnyObject], str: String, funct: (Basic) -> Basic) -> [AnyObject] {
    var argNew:[AnyObject] = []
    var temp: [AnyObject] = []
    var count = 0
    for i in arg {
//        println(count)
        if count == Array(str).count {
            argNew += [funct(convert([i]))]
            count = 0; temp = []; continue
        }
        
//        println(i.dynamicType.description())
        if let i1 = i as? CharObj {
//            println("yay")
            if i1.val == str[count] {
                count += 1
                temp += [i]
...
//        for (pos,j) in enumerate(str) {
//            if let i1 = i as? Character {
//                if i1 == j && count == pos {
//                    count = pos+1; continue
//                }
//            }
//        }
...
func fLog(arg: Basic) -> Basic {
    return Log(arg)
}
func fSin(arg: Basic) -> Basic {
    return Sin(arg)
}
func fCos(arg: Basic) -> Basic {
    return Cos(arg)
}

func strFuncs(arg: [AnyObject]) -> [AnyObject] {
    var argNew = arg
    argNew = strExtract(argNew, "log", fLog)
    argNew = strExtract(argNew, "ln", fLog)
    argNew = strExtract(argNew, "sin", fSin)
    argNew = strExtract(argNew, "cos", fCos)
    return argNew
//    var argNew:[AnyObject] = []
//    var count = 0
//    for (pos,i) in enumerate(arg) {
//        if let i1 = i as? String {
//            if i1 == "l" && count == 0 {
//                count = 1; continue
//            }
//        }
